...
- **Per-student recording default preference (Sarah, Apr 24).** Currently the workspace defaults to *"show live + record"* on every new whiteboard session. Sarah wants the default to be **per-student-overridable** — *"if you know a student doesn't want to record it you can make the default appropriate to their situation."* Add `Student.recordingDefault: "record" | "live_only"` (default `"record"` matching today's behavior); render as a setting on the student edit page ("Record whiteboard sessions by default — yes / no, off-by-default for parents who declined consent"). The whiteboard "start session" flow reads this when initializing the new `WhiteboardSession` row. Tutor can still override per-session at the *Start* moment; the default just biases the initial checkbox state. **Migration:** schema field + ~30-line UI addition + the default flows through the session-create action. **Scope:** ~1 hr. Coordinate with the consent flow (B2) — if consent says "no recording," this should hard-disable, not just default-off.
- **Whiteboard undo (mark removal) — touch + visible button (Sarah, Apr 24).** Excalidraw ships Ctrl/Cmd-Z keyboard shortcut, which works on the tutor's desktop. Two gaps Sarah's ask exposes: (1) **touch-only devices** (her Chromebook students, future iPad students) have no keyboard so Ctrl-Z isn't reachable; (2) the existing toolbar may not surface an explicit undo button on the student canvas. Audit both surfaces (`<WhiteboardLiveCanvas />` for the tutor and student-side equivalent), confirm the standard Excalidraw toolbar undo/redo buttons are visible (Excalidraw exposes them via `excalidrawAPI.history.undo()` programmatically), and add an explicit `<button aria-label="Undo">` if the bundled toolbar is hidden in our integration. Pair with **redo** while we're in there. **Acceptance:** tutor + student can undo their own most-recent stroke via a visible button on touch and desktop, no keyboard required. Tested cross-side: tutor undoes a tutor stroke (works), student undoes a student stroke (works), tutor cannot accidentally undo a student stroke (Excalidraw history is per-client by default — verify this is preserved through our sync layer). **Scope:** ~30–60 min if the toolbar button just needs to be re-enabled; ~2 hr if we have to wire up `excalidrawAPI.history.undo()` to a custom button.
