...
- ~~One `formatDurationMs` (`H:MM:SS`) → `src/lib/time/` (dupes: `replay-helpers.ts:55`, `WhiteboardReplay.tsx:1586`, `WhiteboardWorkspaceClient` local, plus page-local `formatDuration`s). Reconcile with existing `components/recording/format-duration.ts`.~~ → **✅ DONE** (`feat/dedupe-wave-a-format-duration-tail`) — canonical `formatDurationMs` / `formatClockDuration` in `src/lib/time/format-duration-ms.ts`; page-local `formatDuration` copies folded; recording `format-duration.ts` stays separate padded contract.
