...
- **🟡 Whiteboard — "follow my view" + sync student page + tabs/pages (Wyzant-shaped).** *Context:* Wyzant’s room uses **tabs** instead of a freely draggable canvas partly because each person’s pan/zoom is local — pulling the other person to your camera / worksheet **page** is a separate, intentional affordance. **Product bet:** **ship both** (tutor **"snap to my view"** / scroll+zoom, **and** **"follow my tab"** / switch student to tutor’s page when they change tabs) so **Sarah can try them in real sessions**; **pull one or soften defaults** if pilot feedback says one mode is noise or disorienting — no need to pick a single winning affordance up front. **malmesae (Slack, co-designer thread with Jarek, ~Apr 24 2026):** on those tutor-driven controls she’s positive about trying it in product; her words: *"Yeah, it's one of those I might have to try and see what I like."* **Sarah (pilot):** the same Wyzant / whiteboard arc — **tabs** and **staying on the same worksheet** as the tutor — is already in pilot bullets above; this item is the **wire + UX** to match that intent (view + page sync + first-class pages). **Not shipped today:** `sync-client` is **elements-only** (`WhiteboardWireMessage`); no `appState` (scroll/zoom) and no first-class **page** model. **Spec sketch:** (1) **"Snap joiner to my view"** via a small wire payload `{ scrollX, scrollY, zoom }` + `updateScene` on the peer (tutor-triggered; student opt-out / ask-first TBD); (2) **page/tab model** (multiple scenes or named regions) + **"Follow my tab"** so `activePageId` syncs when the tutor changes; (3) **continuous** live camera follow is later / off by default — one-shot snap first. **Estimate:** real feature — schema + relay messages + Excalidraw API + policy UX.
