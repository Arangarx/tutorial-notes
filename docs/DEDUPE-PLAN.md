...
- ~~Retire `SubmitButton` wrapper → `Button` + `useFormStatus` (8 call sites).~~ → **✅ DONE** (`feat/dedupe-wave-a-submit-button`) — deleted `src/components/SubmitButton.tsx`; 8 call sites use local `Button` + `useFormStatus` helpers.
