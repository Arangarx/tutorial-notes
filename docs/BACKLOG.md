...
**Sarah — desktop Chrome follow-up (Apr 20, 2026):** OS mic level alone wasn’t the cause — **Voice Recorder + Upload** worked, **in-browser Record** still produced junk. Pattern matches **wrong device / weak signal in Chrome capture**, not Whisper. ✅ **Shipped:** in-tab **Record** UI now has an explicit **device picker** (`enumerateDevices` after permission), a **boost slider** (digital `GainNode`, 0.25×–3.0×, persisted to `localStorage`), and a **live RMS level meter** (rAF-driven, green/yellow/red). Tutors can hit **Test microphone** to preview before committing to a session. Web Audio graph wraps `getUserMedia` so the boosted stream is what `MediaRecorder` encodes; falls back gracefully to the raw stream if `AudioContext` fails (test stubs, very old browsers). Files: `src/lib/mic-recorder-audio.ts`, `src/app/admin/students/[id]/AudioRecordInput.tsx`. Tests: `src/__tests__/mic-recorder-audio.test.ts`; Playwright `tests/smoke/audio-recording.spec.ts` mock updated to provide `enumerateDevices` + a track with `getSettings`.
