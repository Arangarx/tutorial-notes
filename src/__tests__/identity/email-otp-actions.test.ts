...
describe("confirmTotpEnrollment — null totpSecretEnc (email OTP row)", () => {
  it("returns ok:false and does not call decryptTotpSecret when totpSecretEnc is null", async () => {
    setupAuthMocks();
    ...
    const { confirmTotpEnrollment } = await import("@/app/admin/settings/2fa/actions");
    const result = await confirmTotpEnrollment("123456");

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toMatch(/email verification/i);
      expect(result.error).toMatch(/TOTP enrollment/i);
    }
